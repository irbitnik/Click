...
def rand_event(number: int) -> None:
    match number:
        ...
        case 3:
            pag.moveTo(rand.randint(10, 450), rand.randint(300, 600), duration=rand.uniform(0.4, 1), tween=pag.easeInOutCubic)
            t.sleep(rand.uniform(1, 2))
        case 4:
            pag.moveTo(rand.randint(10, 1000), rand.randint(400, 750), duration=rand.uniform(0.4, 1), tween=pag.easeInOutCubic)
            t.sleep(rand.uniform(1, 2))
        case 5:
...
def find_image(image: str, coord: tuple) -> None:
    pag.locateOnScreen(image, grayscale=True, confidence=0.8, region=coord)
